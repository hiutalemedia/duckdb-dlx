...
static int ValidatePrimary(const Value &v) {
	if (v.IsNull())
		throw BinderException("num_primary must not be NULL");
	int n = v.GetValue<int32_t>();
	if (n < 1)
		throw BinderException("num_primary must be >= 1");
	return n;
}
...
static int ValidateSecondary(const Value &v) {
	int n = v.GetValue<int32_t>();
	if (n < 0)
		throw BinderException("num_secondary must be >= 0");
	return n;
}

static void ValidateRowsType(const Value &v) {
	if (v.IsNull())
		throw BinderException("rows must not be NULL");
...
static void FillSolverFromValue(SolverType &solver, const Value &rows_val) {
	for (const auto &row_val : ListValue::GetChildren(rows_val)) {
		...
		if (row_val.type().id() != LogicalTypeId::LIST)
			throw InvalidInputException("dlx: each element of rows must be a LIST(INTEGER)");
		std::vector<int> cols;
		for (const auto &cv : ListValue::GetChildren(row_val))
			if (!cv.IsNull())
				cols.push_back(cv.GetValue<int32_t>());
		solver.add_row(cols);
	}
}
...
struct DLXSolveBindData : public TableFunctionData {
	...
	DLXSolveBindData(int np, int ns, int64_t ms, Value rows)
	    : num_primary(np), num_secondary(ns), max_solutions(ms), rows_val(std::move(rows)) {
	}
};
...
struct DLXSolveState : public GlobalTableFunctionState {
	...
	explicit DLXSolveState(const DLXSolveBindData &bd) : solver(bd.num_primary, bd.num_secondary) {
...
static unique_ptr<FunctionData> DLXSolveBind(ClientContext &, TableFunctionBindInput &input,
	...
	if (it != input.named_parameters.end() && !it->second.IsNull()) {
		...
		if (ms < 1)
			throw BinderException("max_solutions must be >= 1");
	}
...
static unique_ptr<GlobalTableFunctionState> DLXSolveInit(ClientContext &, TableFunctionInitInput &input) {
...
static void DLXSolveScan(ClientContext &, TableFunctionInput &data_p, DataChunk &output) {
...
static unique_ptr<FunctionData> DLXCountBind(ClientContext &, TableFunctionBindInput &input,
                                             vector<LogicalType> &return_types, vector<string> &names) {
	int np = ValidatePrimary(input.inputs[0]);
	ValidateRowsType(input.inputs[1]);
...
static unique_ptr<GlobalTableFunctionState> DLXCountInit(ClientContext &, TableFunctionInitInput &input) {
...
static void DLXCountScan(ClientContext &, TableFunctionInput &data_p, DataChunk &output) {
...
void RegisterDLXFunctions(ExtensionLoader &loader) {
	TableFunction solve_func("dlx_solve",
	                         {LogicalType::INTEGER, LogicalType::LIST(LogicalType::LIST(LogicalType::INTEGER))},
	...
	TableFunction count_func("dlx_count",
	                         {LogicalType::INTEGER, LogicalType::LIST(LogicalType::LIST(LogicalType::INTEGER))},
