...
protected:
	...
	explicit DLXBase(int num_primary, int num_secondary = 0)
		...
		nodes_[0] = {(num_primary > 0 ? num_primary : 0), (num_primary > 0 ? 1 : 0), 0, 0, 0, -1};

		// Primary column headers 1..num_primary (circular through root)
		for (int i = 1; i <= num_primary; i++) {
			nodes_[i] = {i - 1, (i < num_primary ? i + 1 : 0), i, i, i, -1};
		}
		...
		if (num_secondary > 0) {
			...
			for (int i = first; i <= last; i++) {
				nodes_[i] = {(i == first ? last : i - 1), (i == last ? first : i + 1), i, i, i, -1};
			}
		}
	}
...
public:
	explicit Solver(int num_primary, int num_secondary = 0) : DLXBase(num_primary, num_secondary) {
	}

	void solve(std::function<void(const std::vector<int> &)> cb, int64_t max_solutions = -1) {
		sol_.clear();
		found_ = 0;
		max_ = max_solutions;
		cb_ = std::move(cb);
...
public:
	explicit StreamingSolver(int num_primary, int num_secondary = 0) : DLXBase(num_primary, num_secondary) {
	}
